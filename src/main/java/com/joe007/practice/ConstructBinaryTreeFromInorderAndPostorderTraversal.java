 ...
 * 
 * @author zhoujie Start Time : 2013/11/05 : 13:20 End Time : 2013/11/05 : 14:26
 * 
 */
public class ConstructBinaryTreeFromInorderAndPostorderTraversal {
    ...
    public TreeNode buildTree(int[] inorder, int[] postorder) {
        ...
        if (inorderIndex < inorder.length - 1) {
            // right child
            int[] rightChildInorder = Arrays.copyOfRange(inorder,
                    inorderIndex + 1, inorder.length);
            int[] rightChildPostorder = Arrays.copyOfRange(postorder,
        ...
        if (inorderIndex > 0) {
            // left child
            int[] lefttChildInorder = Arrays.copyOfRange(inorder, 0,
                    inorderIndex);
            int[] leftChildPostorder = Arrays.copyOfRange(postorder, 0,
                    inorderIndex);
